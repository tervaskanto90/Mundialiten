...
const expectPts: Record<StageId, [number, number]> = {
  group: [3, 1], r32: [4, 2], r16: [5, 2], qf: [6, 3], sf: [8, 4], third: [10, 5], final: [10, 5],
}
for (const st of Object.keys(expectPts) as StageId[]) {
  check(`STAGE_POINTS ${st}`, STAGE_POINTS[st].exact === expectPts[st][0] && STAGE_POINTS[st].tendency === expectPts[st][1])
}
