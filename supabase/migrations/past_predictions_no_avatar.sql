-- ─────────────────────────────────────────────────────────────────────────────
-- FIX EGRESS: past_predictions deja de devolver `avatar_url`.
--
-- La RPC devuelve UNA fila por cada (usuario × partido visible). Al traer el
-- avatar (un data URL de ~10 KB) en CADA fila, la misma foto viajaba duplicada
-- decenas de veces por consulta → ~10 MB por llamada. Con el ranking polleando
-- cada minuto, eso reventó la cuota de Egress de Supabase.
--
-- Los marcadores (números) pesan nada; el avatar se baja aparte UNA sola vez
-- (fetchAvatars en el cliente) y se reutiliza. Se mantiene el join a scores sólo
-- para el display_name.
--
-- El tipo de retorno CAMBIA (saca una columna) → hay que DROPEAR primero.
