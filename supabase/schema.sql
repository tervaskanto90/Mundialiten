...
as $$
  ...
  -- Partidos "visibles": ya jugados en el real, O que YA ARRANCARON (kickoff
  -- pasado). Las predicciones se cierran 5 min antes del inicio, así que mostrar
  -- las de un partido que ya arrancó es seguro (nadie puede copiar). Esto hace
  -- que dos partidos EN VIVO simultáneos muestren ambas predicciones, sin
  -- depender de que el flag "played" se haya propagado a la base.
  visible as (
